...
STATIC int mat_shiftrows(MATrec *mat, int *bbase, int delta, LLrec *varmap)
{
  ...
  if(delta > 0) {
    ...
    if(preparecompact) {
      ...
      int *newrowidx = NULL;
      allocINT(mat->lp, &newrowidx, mat->rows+1, _FALSE);
      newrowidx[0] = 0;
      delta = 0;
...
STATIC int mat_mapreplace(MATrec *mat, LLrec *rowmap, LLrec *colmap, MATrec *mat2)
{
  ...
  if(mat2 != NULL) {
    jj = mat2->col_tag[0];
    allocINT(lp, &indirect, jj+1, _FALSE);
    indirect[0] = jj;
    for(i = 1; i <= jj; i++)
      indirect[i] = i;
    hpsortex(mat2->col_tag, jj, 1, sizeof(*indirect), _FALSE, compareINT, indirect);
  }
...
STATIC MATrec *mat_extractmat(MATrec *mat, LLrec *rowmap, LLrec *colmap, MYBOOL negated)
{
  ...
  for(xa = 0; xa < na; xa++, rownr += matRowColStep, colnr += matRowColStep, value += matValueStep) {
    if((isActiveLink(colmap, *colnr) ^ negated) &&
      ...
      mat_setvalue(newmat, *rownr, *colnr, *value, _FALSE);
  }
...
STATIC MYBOOL mat_setcol(MATrec *mat, int colno, int count, REAL *column, int *rowno, MYBOOL doscale, MYBOOL checkrowmode)
{
  ...
  if(checkrowmode && mat->is_roworder)
    return( mat_setrow(mat, colno, count, column, rowno, doscale, _FALSE) );
...
STATIC MYBOOL mat_mergemat(MATrec *target, MATrec *source, MYBOOL usecolmap)
{
  ...
  if((target->rows < source->rows) || !allocREAL(lp, &colvalue, target->rows+1, _FALSE))
    return( _FALSE );

  if(usecolmap) {
    n = source->col_tag[0];
    allocINT(lp, &colmap, n+1, _FALSE);
    for(i = 1; i <= n; i++)
      colmap[i] = i;
    hpsortex(source->col_tag, n, 1, sizeof(*colmap), _FALSE, compareINT, colmap);
  }
  else
    n = source->columns;
  for(i = 1; i <= n; i++) {
    ...
    if(usecolmap) {
      ...
      ix = iy = i;
    mat_expandcolumn(source, ix, colvalue, NULL, _FALSE);
    mat_setcol(target, iy, 0, colvalue, NULL, _FALSE, _FALSE);
  }
...
STATIC MYBOOL mat_setrow(MATrec *mat, int rowno, int count, REAL *row, int *colno, MYBOOL doscale, MYBOOL checkrowmode)
{
  ...
  /* Check if we are in row order mode and should add as column instead;
     the matrix will be transposed at a later stage */
  if(checkrowmode && mat->is_roworder)
    return( mat_setcol(mat, rowno, count, row, colno, doscale, _FALSE) );
...
STATIC int mat_appendrow(MATrec *mat, int count, REAL *row, int *colno, REAL mult, MYBOOL checkrowmode)
{
  ...
  /* Check if we are in row order mode and should add as column instead;
     the matrix will be transposed at a later stage */
  if(checkrowmode && mat->is_roworder)
    return( mat_appendcol(mat, count, row, colno, mult, _FALSE) );
...
STATIC int mat_appendcol(MATrec *mat, int count, REAL *column, int *rowno, REAL mult, MYBOOL checkrowmode)
{
  ...
  /* Check if we are in row order mode and should add as row instead;
     the matrix will be transposed at a later stage */
  if(checkrowmode && mat->is_roworder)
    return( mat_appendrow(mat, count, column, rowno, mult, _FALSE) );
...
{
  return( mat_setvalue(mat, row, column, value, _FALSE) );
}
...
STATIC MYBOOL mat_setvalue(MATrec *mat, int Row, int Column, REAL Value, MYBOOL doscale)
{
  ...
  i = mat_findins(mat, Row, Column, &elmnr, _FALSE);
  if(i == -1)
    return(_FALSE);
  ...
  if(mat_validate(mat)) {
    ...
    for(; bj1 < ej1; bj1++, bj2++) {
      if(COL_MAT_COLNR(bj1) != COL_MAT_COLNR(bj2))
        break;
#if 1
      if(fabs(get_mat_byindex(mat->lp, bj1, TRUE, _FALSE)-get_mat_byindex(mat->lp, bj2, TRUE, _FALSE)) > mat->lp->epsprimal)
#else
...
STATIC MYBOOL mat_transpose(MATrec *mat)
{
  ...
  if(status) {
    ...
    if(nz > 0) {
      ...
      REAL *newValue = NULL;
      int  *newRownr = NULL;
      allocREAL(mat->lp, &newValue, mat->mat_alloc, _FALSE);
      allocINT(mat->lp, &newRownr, mat->mat_alloc, _FALSE);
...
STATIC MYBOOL appendUndoPresolve(lprec *lp, MYBOOL isprimal, REAL beta, int colnrDep)
{
  ...
  if((colnrDep > 0) && (beta != 0) &&
    ...
    if(colnrDep <= lp->columns)
      mat_setvalue(mat, colnrDep, ix, beta, _FALSE);

    /* Handle case where a slack variable is referenced */
    else {
      int ipos, jx = mat->col_tag[ix];
      mat_setvalue(mat, jx, ix, beta, _FALSE);
      jx = mat_findins(mat, jx, ix, &ipos, _FALSE);
...
STATIC MYBOOL addUndoPresolve(lprec *lp, MYBOOL isprimal, int colnrElim, REAL alpha, REAL beta, int colnrDep)
{
  ...
  if(isprimal) {
    ...
    if(*DV == NULL) {
      *DV = createUndoLadder(lp, lp->columns+1, lp->columns);
      ...
      allocINT(lp, &(mat->col_tag), lp->columns+1, _FALSE);
      mat->col_tag[0] = 0;
    }
  }
  else {
    ...
    if(*DV == NULL) {
      *DV = createUndoLadder(lp, lp->rows+1, lp->rows);
      mat = (*DV)->tracker;
      mat->epsvalue = lp->matA->epsvalue;
      allocINT(lp, &(mat->col_tag), lp->rows+1, _FALSE);
      mat->col_tag[0] = 0;
    }
  }
  mat = (*DV)->tracker;
  ...
  if(alpha != 0)
    mat_setvalue(mat, 0, ix, alpha, _FALSE);
...
/*
   ...
   (textbook) format by setting the obj_in_B parameter to _FALSE.
...
Cleanup:
  ...
  test = get_refactfrequency(lp, _FALSE);
...
STATIC MYBOOL fimprove(lprec *lp, REAL *pcol, int *nzidx, REAL roundzero)
{
  ...
  allocREAL(lp, &errors, lp->rows + 1, _FALSE);
  if(errors == NULL) {
    Ok = _FALSE;
    return(Ok);
  }
...
STATIC MYBOOL bimprove(lprec *lp, REAL *rhsvector, int *nzidx, REAL roundzero)
{
  ...
  allocREAL(lp, &errors, lp->sum + 1, _FALSE);
  if(errors == NULL) {
    Ok = _FALSE;
    return(Ok);
  }
...
STATIC MYBOOL get_colIndexA(lprec *lp, int varset, int *colindex, MYBOOL append)
{
  ...
  if(varset & SCAN_PARTIALBLOCK) {
    SETMAX(vb, partial_blockStart(lp, _FALSE));
    SETMIN(ve, partial_blockEnd(lp, _FALSE));
  }
...
/* prod_Ax is only used in fimprove; note that it is NOT VALIDATED/verified as of 20030801 - KE */
{
  ...
  MYBOOL   localset, localnz = _FALSE, isRC;
  MATrec   *mat = lp->matA;
  REAL     sdp;
  REAL     *value;
  int      *rownr;
  ...
  if(localset) {
    ...
    if(!get_colIndexA(lp, varset, coltarget, _FALSE)) {
  ...
  if(localset)
    mempool_releaseVector(lp->workarrays, (char *) coltarget, _FALSE);
  if(localnz)
    mempool_releaseVector(lp->workarrays, (char *) nzinput, _FALSE);
   ...
   the same vector as input, without overwriting the [0..rows] elements. */
{
  ...
  MYBOOL   localset, localnz = _FALSE, includeOF, isRC;
  REALXP   vmax;
  register REALXP v;
