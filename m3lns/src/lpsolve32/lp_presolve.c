...
STATIC MYBOOL presolve_setOrig(lprec *lp, int orig_rows, int orig_cols)
{
  ...
  if(lp->wasPresolved)
    presolve_fillUndo(lp, orig_rows, orig_cols, _FALSE);
  return( TRUE );
}
...
STATIC void presolve_storeDualUndo(presolverec *psdata, int rownr, int colnr)
{
  ...
  for(ix = presolve_nextrow(psdata, colnr, &item); ix >= 0;
    ...
    if(iix == rownr)
      continue;
    if(!firstdone)
      firstdone = addUndoPresolve(lp, _FALSE, rownr, get_mat(lp, 0, colnr)/Aij,
                                                    get_mat_byindex(lp, ix, _FALSE, TRUE)/Aij, iix);
    else
      appendUndoPresolve(lp, _FALSE, get_mat_byindex(lp, ix, _FALSE, TRUE)/Aij, iix);
  }
}
...
{
  *loValue = presolve_sumplumin(lp, rownr,   ps, _FALSE);
...
STATIC void presolve_rangeorig(lprec *lp, int rownr, psrec *ps, REAL *loValue, REAL *hiValue, REAL delta)
{
  ...
  *loValue = presolve_sumplumin(lp, rownr,   ps, _FALSE) + delta;
...
STATIC MYBOOL presolve_rowfeasible(presolverec *psdata, int rownr, MYBOOL userowmap)
{
  ...
  while((status == TRUE) && (rownr != 0)) {
    ...
    value = presolve_sumplumin(lp, rownr, psdata->rows, _FALSE);
...
#else
    lp->orig_rhs[rownr] = presolve_roundrhs(lp, lp->orig_rhs[rownr], _FALSE);
#endif
  ...
  if(list != NULL) {
    ...
    for(i = 1; i <= n; i++)
      if(isActiveLink(psdata->rows->varmap, list[i])) {
        presolve_rowremove(psdata, list[i], _FALSE);
        countR++;
      }
    if(nConRemove != NULL)
      (*nConRemove) += countR;
    ...
    for(i = 1; i <= n; i++) {
      ...
      if(isActiveLink(psdata->cols->varmap, ix)) {
        if(presolve_colfixdual(psdata, ix, &fixValue, &status)) {
          ...
          presolve_colremove(psdata, ix, _FALSE);
          countC++;
        }
...
STATIC int presolve_redundantSOS(presolverec *psdata, int *nb, int *nSum)
{
  ...
  if(!allocINT(lp, &fixed, lp->columns+1, _FALSE) )
...
STATIC MYBOOL presolve_fixSOS1(presolverec *psdata, int colnr, REAL fixvalue, int *nr, int *nv)
{
  ...
  SOSrec   *SOS;
  REAL     newvalue;
  MYBOOL   *fixed = NULL, status = _FALSE;
...
{
  lprec    *lp = psdata->lp;
  MYBOOL   rowbindsvar = _FALSE, status = _FALSE;
  ...
  if(!my_infinite(lp, LHS) && !my_infinite(lp, netX)) {
    if(coeff_a > 0) {
      ...
      if(LHS < Xupper - epsvalue) {
        Xupper = presolve_roundrhs(lp, LHS, _FALSE);
        status = AUTOMATIC;
      }
  ...
  netX = presolve_sumplumin(lp, rownr, psdata->rows, _FALSE);
  if(!my_infinite(lp, RHS) && !my_infinite(lp, netX)) {
    if(coeff_a < 0) {
      ...
      if(RHS < Xupper - epsvalue) {
        Xupper = presolve_roundrhs(lp, RHS, _FALSE);
        status |= AUTOMATIC;
      }
...
STATIC MYBOOL presolve_rowfix(presolverec *psdata, int rownr, REAL newvalue, MYBOOL remove, int *tally)
{
  lprec    *lp = psdata->lp;
  int      i, ix, ie;
  MYBOOL   isneg, lofinite, upfinite, doupdate = _FALSE, chsign = is_chsign(lp, rownr);
  ...
  if(remove) {
    ...
    if(fixvalue != 0)
      addUndoPresolve(lp, _FALSE, rownr, fixvalue, 0, 0);
    mult = -1;
  }
  else {
    mult = 1;
   ...
   /* Cumulate effective upper column bound (only bother with non-finite bound) */
    if(isneg) {
      if((ps->negupper[i] < lp->infinite) && lofinite) {
        ...
        ps->negupper[i] = presolve_roundrhs(lp, ps->negupper[i], _FALSE);
      }
      ...
      if((ps->pluupper[i] < lp->infinite) && upfinite) {
        ...
        ps->pluupper[i] = presolve_roundrhs(lp, ps->pluupper[i], _FALSE);
      }
...
STATIC MYBOOL presolve_colfix(presolverec *psdata, int colnr, REAL newvalue, MYBOOL remove, int *tally)
{
  lprec    *lp = psdata->lp;
  int      i, ix, ie;
  MYBOOL   isneg, lofinite, upfinite, doupdate = _FALSE, doOF = TRUE;
  REAL     lobound, upbound, lovalue, upvalue,
           Value, fixvalue, mult;
  MATrec   *mat = lp->matA;
  ...
  for(; (ix < ie) && isDualFREE;
    ...
    if(presolve_rowlength(psdata, i) == 1) {
      ...
      if(upR < upX - psdata->epsvalue)
        upX = presolve_roundrhs(lp, upR, _FALSE);
      continue;
    }
    else
      isDualFREE = my_infinite(lp, get_rh_range(lp, i)) ||                                          /* Explicitly free */
                   ((presolve_sumplumin(lp, i, psdata->rows, TRUE)-eps <= get_rh_upper(lp, i)) &&   /* Implicitly free */
                    (presolve_sumplumin(lp, i, psdata->rows, _FALSE)+eps >= get_rh_lower(lp, i)));
    if(isDualFREE) {
...
STATIC MYBOOL presolve_probefix01(presolverec *psdata, int colnr, REAL *fixvalue)
{
  ...
  for(ix = presolve_nextrow(psdata, colnr, &item); (ix >= 0); ix = presolve_nextrow(psdata, colnr, &item)) {
    ...
    *fixvalue = COL_MAT_VALUE(ix);
    ...
    loLim = presolve_sumplumin(lp, i, psdata->rows, _FALSE);
...
STATIC int presolve_mergerows(presolverec *psdata, int *nRows, int *nSum)
{
  ...
  for(i = lastActiveLink(psdata->rows->varmap); (i > 0) && (status == RUNNING); ) {
    ...
    for(RT1 = 0; (ix > 0) && (RT1 < RT2) && (status == RUNNING);
      ...
      Value1 = get_mat_byindex(lp, iix, TRUE, _FALSE);
      Value2 = get_mat_byindex(lp, jjx, TRUE, _FALSE);
      bound = Value1 / Value2;
      Value1 = bound;
...
STATIC int presolve_knapsack(presolverec *psdata, int *nn)
{
  ...
  /* Get the OF row */
  allocINT(lp, &rownr,  map->count+1, _FALSE);
  allocREAL(lp, &ratio, map->count+1, _FALSE);
...
STATIC int presolve_singularities(presolverec *psdata, int *nn, int *nr, int *nv, int *nSum)
{
  ...
  allocINT(lp, &rmapout, psdata->EQmap->count+1, _FALSE);
  allocINT(lp, &cmapout, lp->columns+1, _FALSE);
  n = 0;
...
STATIC int presolve_elimeq2(presolverec *psdata, int *nn, int *nr, int *nc, int *nSum)
{
  ...
  if((EQ2 == NULL) || !allocREAL(lp, &colvalue, nrows+1, _FALSE) ||
                      !allocREAL(lp, &delvalue, nrows+1, _FALSE))
    goto Finish;
  for(i = firstActiveLink(psdata->EQmap); i > 0; i = nextActiveLink(psdata->EQmap, i)) {
    if(presolve_rowlength(psdata, i) == 2)
      appendLink(EQ2, i);
  ...
  for(i = firstActiveLink(EQ2); i > 0; i = nextActiveLink(EQ2, i)) {
    ...
    if(freshupdate)
      ...
      mat_expandcolumn(rev, colindex[jjx], colvalue, NULL, _FALSE);
    if((colindex == NULL) || (colindex[jx] == 0))
      mat_expandcolumn(mat, jx, delvalue, NULL, TRUE);
    else
      mat_expandcolumn(rev, colindex[jx], delvalue, NULL, _FALSE);
      ...
      if(Value2 > 0) {
        ...
        if(bound < test - psdata->epsvalue)
          if(is_int(lp, jjx))
            ...
            lp->orig_upbo[k] = presolve_roundrhs(lp, bound, _FALSE);
      }
      else {
    ...
    if(rev == NULL) {
      ...
      allocINT(lp, &(rev->col_tag), mat->columns_alloc+1, _FALSE);
      allocINT(lp, &colindex, lp->columns+1, TRUE);
      rev->col_tag[0] = 0;
    }
    n = rev->col_tag[0] = incrementUndoLadder(DV);
    mat_setcol(rev, n, 0, colvalue, NULL, _FALSE, _FALSE);
    rev->col_tag[n] = jjx;
    ...
    jx = presolve_colremove(psdata, jx, _FALSE);
    iVarsFixed++;
    ...
    if(item == 0) {
      ...
      if(presolve_colfix(psdata, jjx, 0.0, TRUE, nc))
        jjx = presolve_colremove(psdata, jjx, _FALSE);
    }

    /* Delete the row */
    presolve_rowremove(psdata, i, _FALSE);
    iRowsRemoved++;
  }
...
STATIC MYBOOL presolve_impliedfree(lprec *lp, presolverec *psdata, int colnr)
{
  int    i, ix, ie;
  REAL   Tlower, Tupper;
  MYBOOL status, rowbinds, isfree = _FALSE;
  MATrec *mat = lp->matA;
...
{
  lprec    *lp = psdata->lp;
  MYBOOL   signflip, undoadded = _FALSE;
  MATrec   *mat = lp->matA;
  ...
  if(isfree && is_constr_type(lp, rownr, EQ)) {
    ...
    if(isfree) {
      ...
      matValue = presolve_sumplumin(lp, rownr, psdata->rows, _FALSE);
    ...
    if(isfree) {
      ...
      /* Add undo information for the dual of the deleted constraint */
      if(dual != 0)
        addUndoPresolve(lp, _FALSE, rownr, dual, 0.0, 0);
    }
...
STATIC psrec *presolve_initpsrec(lprec *lp, int size)
{
  ...
  allocINT(lp, &ps->empty, size, _FALSE);
  ps->empty[0] = 0;

  allocREAL(lp, &ps->pluupper,  size, _FALSE);
  allocREAL(lp, &ps->negupper,  size, _FALSE);
  allocREAL(lp, &ps->plulower,  size, _FALSE);
  allocREAL(lp, &ps->neglower,  size, _FALSE);
  allocINT(lp,  &ps->infcount,  size, _FALSE);
...
STATIC presolverec *presolve_init(lprec *lp)
{
  ...
  k = lp->sum + 1;
  allocREAL(lp, &psdata->pv_lobo, k, _FALSE);
  MEMCOPY(psdata->pv_lobo, lp->orig_lowbo, k);
  allocREAL(lp, &psdata->pv_upbo, k, _FALSE);
  MEMCOPY(psdata->pv_upbo, lp->orig_upbo, k);

  /* Create and initialize dual value (Langrangean and slack) limits */
  allocREAL(lp, &psdata->dv_lobo, k, _FALSE);
  allocREAL(lp, &psdata->dv_upbo, k, _FALSE);
  for(i = 0; i <= nrows; i++) {
...
STATIC MYBOOL presolve_updatesums(presolverec *psdata)
{
  ...
  for(j = firstActiveLink(psdata->cols->varmap); j != 0;
    ...
    presolve_colfix(psdata, j, lp->infinite, _FALSE, NULL);
  }
  ...
  for(j = firstActiveLink(psdata->rows->varmap); j != 0;
    ...
    presolve_rowfix(psdata, j, lp->infinite, _FALSE, NULL);
  }
#endif
...
STATIC MYBOOL presolve_finalize(presolverec *psdata)
{
  ...
  lp->presolve_undo->OFcolsdeleted = _FALSE;
...
STATIC int presolve_rowdominance(presolverec *psdata, int *nCoeffChanged, int *nRowsRemoved, int *nVarsFixed, int *nSum)
{
  ...
  if(!allocREAL(lp, &rowvalues, lp->columns + 1, TRUE) ||
     !allocINT(lp, &coldel, lp->columns + 1, _FALSE))
    goto Finish;
   ...
   that are dominated via set coverage or unit knapsack constraints */
{
  ...
  if(!allocREAL(lp, &colvalues, nrows + 1, TRUE) ||
     !allocREAL(lp, &colobj, n + 1, _FALSE) ||
     !allocINT(lp, &coldel, n + 1, _FALSE))
    goto Finish;
  ...
  for(ib = 0; ib < n; ib++) {
    ...
    if(coldel[0] > 1) {
      qsortex(colobj+1, coldel[0], 0, sizeof(*colobj), _FALSE, compareREAL, coldel+1, sizeof(*coldel));
...
STATIC int presolve_makesparser(presolverec *psdata, int *nCoeffChanged, int *nConRemove, int *nVarFixed, int *nSum)
{
  ...
  allocINT(lp, &nzidx, lp->columns + 1, _FALSE);
...
{
  lprec    *lp = psdata->lp;
  MYBOOL   candelete, SOS_GUBactive = _FALSE;
...
STATIC int presolve_freeandslacks(presolverec *psdata, int *nCoeffChanged, int *nConRemove, int *nVarFixed, int *nSum)
{
  ...
  for(j = firstActiveLink(psdata->cols->varmap); j != 0; ) {
    ...
    /* Check for implied slack variable in equality constraint */
             ...
             presolve_impliedcolfix(psdata, i, j, _FALSE)) {
      ...
      if(ValueA > 0) {
        ...
        if(!my_infinite(lp, *target)) {
          if(my_infinite(lp, coeff_bu)) {
            ...
            *target = presolve_roundrhs(lp, *target, _FALSE);
          }
        }
      }
   ...
   /* First identify any full row infeasibilities */
...
#else
    if((j > 1) && !psdata->forceupdate && !presolve_rowfeasible(psdata, i, _FALSE)) {
#endif
      status = presolve_setstatus(psdata, INFEASIBLE);
      break;
    }
    ...
    if(impliedfree && (j > 1) && mat_validate(mat)){
      ...
      if(upsum < uprhs-epsvalue) {
        set_rh_upper(lp, i, presolve_roundrhs(lp, upsum, _FALSE));
        iRangeTighten++;
      }
    }
...
#if 1
      if(j > 1)
        status = presolve_rowtighten(psdata, i, &iBoundTighten, _FALSE);
#else
...
STATIC int presolve_rows(presolverec *psdata, int *nCoeffChanged, int *nConRemove, int *nVarFixed, int *nBoundTighten, int *nSum)
{
  ...
  for(i = lastActiveLink(psdata->rows->varmap); (i > 0) && (status == RUNNING); ) {
   ...
   /* First identify any full row infeasibilities
    ...
    j = presolve_rowlengthex(psdata, i);
    if((j > 1) &&
       !psdata->forceupdate && !presolve_rowfeasible(psdata, i, _FALSE)) {
      status = presolve_setstatus(psdata, INFEASIBLE);
      break;
    }
    presolve_range(lp, i, psdata->rows, &losum, &upsum);
    ...
    if((j == 1) && (uprhs-lorhs >= -epsvalue)) {
      ...
      if(status == RUNNING) {
        if((fabs(Value2-Value1) < epsvalue) && (fabs(Value2) > epsvalue)) {
          MYBOOL isSOS     = (MYBOOL) (SOS_is_member(lp->SOS, 0, j) != _FALSE),
...
/*           !is_presolve(lp, PRESOLVE_ELIMEQ2) && */
...
#if 0
        REPORT_mat_mmsave(lp, "A.mtx", NULL, _FALSE, "Constraint matrix A");
#endif
  ...
  if(status == RUNNING) {
    ...
    if(nConRemove+nVarFixed+nBoundTighten+nVarFixed+nCoeffChanged > 0) {
      REPORT_modelinfo(lp, _FALSE, "REDUCED");
      if(nSum > 0) {
...
#endif
#if 0
  REPORT_debugdump(lp, "testint2.txt", _FALSE);
#endif
...
#if 0
  REPORT_mat_mmsave(lp, "basis.mtx", NULL, _FALSE);  /* Write the current basis matrix (no OF) */
#endif
