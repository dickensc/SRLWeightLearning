...
int CurtisReidScales(lprec *lp, MYBOOL _Advanced, REAL *FRowScale, REAL *FColScale)
{
  ...
  allocREAL(lp, &RowScalem2, lp->rows+1, _FALSE);
  allocREAL(lp, &ColScalem2, colMax+1, _FALSE);
...
STATIC MYBOOL scaleCR(lprec *lp, REAL *scaledelta)
{
  ...
  if(!lp->scaling_used) {
    allocREAL(lp, &lp->scalars, lp->sum_alloc + 1, _FALSE);
  ...
  if(scaledelta == NULL)
    allocREAL(lp, &scalechange, lp->sum + 1, _FALSE);
  else
    scalechange = scaledelta;

  Result=CurtisReidScales(lp, _FALSE, scalechange, &scalechange[lp->rows]);
  if(Result>0) {
...
STATIC REAL scale(lprec *lp, REAL *scaledelta)
{
 ...
 /* Must initialize due to computation of scaling statistic - KE */
  ...
  allocREAL(lp, &row_min, row_count + 1, _FALSE);
...
STATIC REAL auto_scale(lprec *lp)
{
  ...
  if(lp->scalemode != SCALE_NONE) {
    ...
    if((lp->solvecount > 1) && (lp->bb_level < 1) &&
      ...
      allocREAL(lp, &scalenew, lp->sum + 1, _FALSE);
