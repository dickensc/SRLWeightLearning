   ...
   Allowed variable set: Any pivot PRIMAL:larger or DUAL:smaller than threshold value of 0 */
{
  ...
  if(Accept) {
    if(candidatecount != NULL)
      (*candidatecount)++;
    if(collectMP) {
      if(addCandidateVar(candidate, current->lp->multivars, (findCompare_func *) compareImprovementQS, _FALSE) < 0)
        return(Action);
    }
    if(current->varno > 0)
...
STATIC int colprim(lprec *lp, REAL *drow, int *nzdrow, MYBOOL skipupdate, int partialloop, int *candidatecount, MYBOOL updateinfeas, REAL *xviol)
{
  ...
  current.varno    = 0;
  current.lp       = lp;
  current.isdual   = _FALSE;
  candidate.lp     = lp;
  candidate.isdual = _FALSE;
  *candidatecount  = 0;
...
doLoop:
  ...
  if((lp->multivars != NULL) && ((lp->simplex_mode & SIMPLEX_PRIMAL_PRIMAL) != 0)) {
    ...
    if(collectMP) {
      ...
      coltarget = multi_indexSet(lp->multivars, _FALSE);
  }
...
#endif
    compute_reducedcosts(lp, _FALSE, 0, coltarget, (MYBOOL) ((nloop <= 1) || (partialloop > 1)),
                             NULL, NULL,
                             drow, nzdrow,
                             MAT_ROUNDDEFAULT);
  }
  ...
  for(; ix*iz <= iy; ix += iz) {
   ...
   /* Find entering variable according to strategy (largest positive f) */
    ...
    SETMAX(xinfeas, f);
    sinfeas += f;
    candidate.pivot = normalizeEdge(lp, i, f, _FALSE);
    candidate.varno = i;
  ...
  if(lp->multivars != NULL) {
    if(collectMP) {
      ...
      ix = partial_blockStart(lp, _FALSE);
      iy = partial_blockEnd(lp, _FALSE);
      lp->multivars->used = 0;
...
STATIC int rowprim(lprec *lp, int colnr, LREAL *theta, REAL *pcol, int *nzpcol, MYBOOL forceoutEQ, REAL *xviol)
{
  ...
  pricerec current, candidate;
  MYBOOL   isupper = !lp->is_lower[colnr], HarrisTwoPass = _FALSE;
...
/*        lp->is_lower[colnr] = _FALSE; */
 ...
 /* Return working array to pool */
  if(nzpcol == NULL)
    mempool_releaseVector(lp->workarrays, (char *) nzlist, _FALSE);
...
/*    if((rh < -epsvalue*sqrt(lp->matA->rowmax[i])) || */
  ...
  if(updateinfeas)
    lp->suminfeas = fabs(sinfeas);
  if((ninfeas > 1) &&
     !verify_stability(lp, _FALSE, xinfeas, sinfeas, ninfeas)) {
                                                    ...
                                                    REAL *drow, int *nzdrow)
{
  ...
  if(which == 0) {             /* Maros Example-1 - raw data */
    ...
    j =  4; i = lp->rows+j; lp->upbo[i] = 1;  lp->is_lower[i] = _FALSE; nzprow[j] = i; prow[i] =  3; drow[i] = -6;
    j =  5; i = lp->rows+j; lp->upbo[i] = 1;  lp->is_lower[i] = _FALSE; nzprow[j] = i; prow[i] = -4; drow[i] = -2;
    j =  6; i = lp->rows+j; lp->upbo[i] = 1;  lp->is_lower[i] =  TRUE; nzprow[j] = i; prow[i] = -1; drow[i] =  0;
    j =  7; i = lp->rows+j; lp->upbo[i] = 2;  lp->is_lower[i] = _FALSE; nzprow[j] = i; prow[i] =  1; drow[i] =  0;
    j =  8; i = lp->rows+j; lp->upbo[i] = 1;  lp->is_lower[i] = _FALSE; nzprow[j] = i; prow[i] = -2; drow[i] =  0;
    ...
    j =  3; i = lp->rows+j; lp->upbo[i] = 1;  lp->is_lower[i] = _FALSE; nzprow[j] = i; prow[i] = -4; drow[i] = -2;
    j =  4; i = lp->rows+j; lp->upbo[i] = 1;  lp->is_lower[i] = _FALSE; nzprow[j] = i; prow[i] = -2; drow[i] =  0;

    j =  5; i = lp->rows+j; lp->upbo[i] = 1;  lp->is_lower[i] =  TRUE; nzprow[j] = i; prow[i] = -1; drow[i] =  0;
    j =  6; i = lp->rows+j; lp->upbo[i] = 2;  lp->is_lower[i] = _FALSE; nzprow[j] = i; prow[i] =  1; drow[i] =  0;
    j =  7; i = lp->rows+j; lp->upbo[i] = 1;  lp->is_lower[i] =  TRUE; nzprow[j] = i; prow[i] = -2; drow[i] =  2;
    j =  8; i = lp->rows+j; lp->upbo[i] = 1;  lp->is_lower[i] = _FALSE; nzprow[j] = i; prow[i] =  3; drow[i] = -6;
    j =  9; i = lp->rows+j; lp->upbo[i] = 5;  lp->is_lower[i] =  TRUE; nzprow[j] = i; prow[i] = -1; drow[i] =  4;
    j = 10; i = lp->rows+j; lp->upbo[i] = F;  lp->is_lower[i] =  TRUE; nzprow[j] = i; prow[i] = -2; drow[i] = 10;
    nzprow[0] = i-lp->rows;
    lp->rhs[rownr] = -11;
    ...
    j =  3; i = lp->rows+j; lp->upbo[i] = 1;  lp->is_lower[i] = _FALSE; nzprow[j] = i; prow[i] = -2; drow[i] =  0;
    j =  4; i = lp->rows+j; lp->upbo[i] = 2;  lp->is_lower[i] = _FALSE; nzprow[j] = i; prow[i] = -1; drow[i] = -2;
...
#endif
  pricerec current, candidate;
  MYBOOL   isbatch = _FALSE, /* Requires that lp->longsteps->size > lp->sum */
...
STATIC int partial_findBlocks(lprec *lp, MYBOOL autodefine, MYBOOL isrow)
{
  ...
  allocREAL(lp, &sum, items+1, _FALSE);
...
STATIC int multi_populateSet(multirec *multi, int **list, int excludenr)
{
  ...
  if(list == NULL)
    list = &(multi->indexSet);
  if((multi->used > 0) &&
     ((*list != NULL) || allocINT(multi->lp, list, multi->size+1, _FALSE))) {
    int i, colnr;
