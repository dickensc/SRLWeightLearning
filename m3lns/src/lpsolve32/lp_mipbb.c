...
STATIC BBrec *create_BB(lprec *lp, BBrec *parentBB, MYBOOL dofullcopy)
{
  ...
  if(newBB != NULL) {
    ...
    if(parentBB == NULL) {
      allocREAL(lp, &newBB->upbo,  lp->sum + 1, _FALSE);
      allocREAL(lp, &newBB->lowbo, lp->sum + 1, _FALSE);
...
/* Push ingoing bounds and B&B data onto the stack */
{
  ...
  newBB = create_BB(lp, parentBB, _FALSE);
  if(newBB != NULL) {
    ...
    /* Otherwise check if we should do automatic branching */
      ...
      /* Set direction by pseudocost (normally used in tandem with NODE_PSEUDOxxxSELECT) */
        ...
        BB->isfloor = (MYBOOL) (get_pseudobranchcost(lp->bb_PseudoCost, k, TRUE) >
                                get_pseudobranchcost(lp->bb_PseudoCost, k, _FALSE));
        if(is_maxim(lp))
...
/*SetUB:*/
    ...
    /* Next, establish the lower bound to be applied (when isfloor == _FALSE)
       --------------------------------------------------------------------- */
SetLB:
...
#else
        perturb_bounds(lp, perturbed, TRUE, TRUE, _FALSE);
#endif
        impose_bounds(lp, perturbed->upbo, perturbed->lowbo);
        set_action(&lp->spx_action, ACTION_REBASE | ACTION_RECOMPUTE);
        BB->UBzerobased = _FALSE;
...
STATIC MYBOOL findnode_BB(BBrec *BB, int *varno, int *vartype, int *varcus)
{
  ...
  REAL   varsol;
  MYBOOL is_better = _FALSE, is_equal = _FALSE, is_feasible = TRUE;
  lprec  *lp = BB->lp;
  ...
  *varcus   = 0;
  ...
  if((lp->bb_limitlevel != 1) && (MIP_count(lp) > 0)) {
    ...
    if((SOS_count(lp) > 0) && (*varno == 0)) {
      *varno = find_sos_bbvar(lp, &countnint, _FALSE);
      if(*varno < 0)
        *varno = 0;
      else if(*varno > 0)
        *vartype = BB_SOS;
...
*/
    ...
    if(*varno == 0) {
      ...
      /* Current solution is better */
        ...
        if(lp->bb_trace ||
           ((lp->verbose >= NORMAL) && (lp->print_sol == _FALSE) && (lp->lag_status != RUNNING))) {
          report(lp, IMPORTANT,
...
/*    set_action(&lp->spx_action, ACTION_REBASE | ACTION_RECOMPUTE); */
    if(solve_BB(strongBB) == OPTIMAL) {
      ...
      for(i = 1; i <= lp->columns; i++) {
        if(is_int(lp, i) && !solution_is_int(lp, lp->rows+i, _FALSE))
          strongBB->lastvarcus++;
      }
