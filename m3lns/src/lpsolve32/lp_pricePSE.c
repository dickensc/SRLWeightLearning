...
STATIC MYBOOL restartPricer(lprec *lp, MYBOOL isdual)
{
  ...
  ok = allocREAL(lp, &sEdge, m+1, _FALSE);
  if(!ok)
    return( ok );
  ...
  if(isdual) {
   ...
   /* Solve a=Bb for b over all non-basic variables and compute their squared norms */
    ...
    for(i = 1; i <= lp->sum; i++) {
      ...
      fsolve(lp, i, sEdge, NULL, 0, 0.0, _FALSE);
...
{
  MYBOOL ok = allocREAL(lp, w, lp->rows+1, _FALSE);

  if(ok) {
    if(pcol == NULL)
      fsolve(lp, colnr, *w, NULL, 0.0, 0.0, _FALSE);
    else {
...
STATIC MYBOOL updatePricer(lprec *lp, int rownr, int colnr, REAL *pcol, REAL *prow, int *nzprow)
{
  ...
  MYBOOL forceRefresh = _FALSE, isDual, isDEVEX, ok = _FALSE;
  ...
  if(isDual) {
    ...
    if(!isDEVEX) {
      ok = allocREAL(lp, &vEdge, m+1, _FALSE);
      if(!ok)
        return( ok );
  ...
  /* Price norms for the primal simplex - the non-basic columns */
    ...
    ok = get_colIndexA(lp, SCAN_SLACKVARS+SCAN_USERVARS+USE_NONBASICVARS, coltarget, _FALSE);
    if(!ok) {
