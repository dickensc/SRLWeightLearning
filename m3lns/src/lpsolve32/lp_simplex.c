...
STATIC MYBOOL stallMonitor_create(lprec *lp, MYBOOL isdual, char *funcname)
{
  ...
  if(MAX_STALLCOUNT <= 1)
    monitor->limitstall[_FALSE] = 0;
  else
    monitor->limitstall[_FALSE] = MAX(MAX_STALLCOUNT,
                                     (int) pow((REAL) (lp->rows+lp->columns)/2, 0.667));
#if 1
  monitor->limitstall[_FALSE] *= 2+2;  /* Expand degeneracy/stalling tolerance range */
#endif
  monitor->limitstall[TRUE] = monitor->limitstall[_FALSE];
  ...
  if(add) {
    ...
    if(add) {
     ...
     /* Create temporary sparse array storage */
      if(nzarray == NULL)
        allocREAL(lp, &avalue, 2, _FALSE);
      else
        avalue = nzarray;
      if(idxarray == NULL)
        allocINT(lp, &rownr, 2, _FALSE);
      else
        rownr = idxarray;
...
#if 0
    performiteration(lp, rownr, colnr, 0.0, TRUE, _FALSE, prow, NULL,
                                                          NULL, NULL, NULL);
#else
...
{
  MYBOOL primal = TRUE, bfpfinal = _FALSE, changedphase = _FALSE, forceoutEQ = AUTOMATIC,
 ...
 /* Add sufficent number of artificial variables to make the problem feasible
  ...
  ok = allocREAL(lp, &(lp->bsolveVal), lp->rows + 1, _FALSE) &&
       allocREAL(lp, &prow, lp->sum + 1, TRUE) &&
       allocREAL(lp, &pcol, lp->rows + 1, TRUE);
  if(is_piv_mode(lp, PRICE_MULTIPLE) && (lp->multiblockdiv > 1)) {
    lp->multivars = multi_create(lp, _FALSE);
    ok &= (lp->multivars != NULL) &&
          multi_resize(lp->multivars, lp->sum / lp->multiblockdiv, 2, _FALSE, TRUE);
  }
  if(!ok)
    goto Finish;
  ...
  pendingunbounded = _FALSE;

  ok = stallMonitor_create(lp, _FALSE, "primloop");
  if(!ok)
    goto Finish;
...
RetryCol:
#if 0
    if(verify_solution(lp, _FALSE, "spx_loop") > 0)
...
Optimality:
      ...
      if((lp->bb_level <= 1) || (lp->improve & IMPROVE_BBSIMPLEX) /* || (lp->bb_rule & NODE_RCOSTFIXING) */) { /* NODE_RCOSTFIXING fix */
        set_action(&lp->piv_strategy, PRICE_FORCEFULL);
          i = rowdual(lp, lp->rhs, _FALSE, _FALSE, NULL);
...
{
  MYBOOL primal = _FALSE, inP1extra, dualphase1 = _FALSE, changedphase = TRUE,
         pricerCanChange, minit, stallaccept, longsteps,
         forceoutEQ = _FALSE, bfpfinal = _FALSE;
  ...
  ok = allocREAL(lp, &prow,   lp->sum + 1,  TRUE) &&
       allocINT (lp, &nzprow, lp->sum + 1,  _FALSE) &&
       allocREAL(lp, &pcol,   lp->rows + 1, TRUE);
  if(!ok)
    goto Finish;
...
#endif
    boundswaps = multi_indexSet(lp->longsteps, _FALSE);
  }
  ...
  /* Check if we have equality slacks in the basis and we should try to
     ...
     forceoutEQ = _FALSE :    Only eliminate assured "good" violated
  ...
  if(is_anti_degen(lp, ANTIDEGEN_DYNAMIC) && (bin_count(lp, TRUE)*2 > lp->columns)) {
    switch (forceoutEQ) {
      case _FALSE:     forceoutEQ = AUTOMATIC;
                      break;
    ...
    /* High quality solution with no leaving candidates available ... */
      ...
      if(inP1extra && (colnr == 0) && (lp->fixedvars > 0) && is_anti_degen(lp, ANTIDEGEN_FIXEDVARS)) {
        ...
        while(lp->fixedvars > 0) {
          ...
          if(colnr > 0) {
            theta = 0;
            performiteration(lp, rownr, colnr, theta, TRUE, _FALSE, prow, NULL,
                                                            NULL, NULL, NULL);
            lp->fixedvars--;
          }
        }
      ...
      /* We are primal feasible and also optimal if we were in phase 2 */
        ...
        if((dualoffset != 0) || (lp->bb_level <= 1) || (lp->improve & IMPROVE_BBSIMPLEX) || (lp->bb_rule & NODE_RCOSTFIXING)) { /* NODE_RCOSTFIXING fix */
          set_action(&lp->piv_strategy, PRICE_FORCEFULL);
            colnr = colprim(lp, drow, nzdrow, _FALSE, 1, &candidatecount, _FALSE, NULL);
          clear_action(&lp->piv_strategy, PRICE_FORCEFULL);
...
#ifdef UseLongStepDualPhase1
  allocINT(lp, &infeasibles, lp->columns + 1, _FALSE);
  infeasibles[0] = 0;
#endif
...
STATIC int lag_solve(lprec *lp, REAL start_bound, int num_iter)
{
  ...
  if(!allocREAL(lp, &OrigObj, lp->columns + 1, _FALSE) ||
