...
MYBOOL __WINAPI MPS_readex(lprec **newlp, void *userhandle, read_modeldata_func read_modeldata, int typeMPS, int verbose)
{
  ...
  MYBOOL Int_section, Column_ready, Column_ready1,
         Unconstrained_rows_found = _FALSE, OF_found = _FALSE, CompleteStatus = _FALSE;
  ...
  if (lp != NULL) {
    ...
    while(read_modeldata(userhandle, line, BUFSIZ - 1)) {
      ...
      if(line[0] != ' ') {
        ...
        case MPSCOLUMNS:
          ...
          if((items == 4) || (items == 5) || (items == 6)) {
            if (NZ == 0)
              ...
              if(Column_ready1) {
                if (find_var(lp, field2, _FALSE) >= 0) {
        ...
        case MPSBOUNDS:
          ...
          var = find_var(lp, field3, _FALSE);
          if(var < 0){ /* bound on undefined var in COLUMNS section ... */
            Column_ready = TRUE;
            if (!addmpscolumn(lp, _FALSE, &Column_ready, &count, Last_column, Last_columnno, field3))
              break;
            Column_ready = TRUE;
      ...
      and a reduced CPLEX-like format.                                       */
        ...
        case MPSSOS:
          ...
          /* Otherwise, add set members to the active SOS */
            ...
            var = find_var(lp, field, _FALSE);  /* Native lp_solve and XPRESS formats */
            if(var < 0){ /* SOS on undefined var in COLUMNS section ... */
              Column_ready = TRUE;
              if (!addmpscolumn(lp, _FALSE, &Column_ready, &count, Last_column, Last_columnno, field))
                break;
              Column_ready = TRUE;
    ...
    if((*OBJNAME) && (!OF_found)) {
      ...
      CompleteStatus = _FALSE;
    }

    if(CompleteStatus == _FALSE)
      delete_lp(lp);
    else
      *newlp = lp;
    if(Last_column != NULL)
...
{
  int    i, j, jj, je, k, marker, putheader, ChangeSignObj = _FALSE, *idx, *idx1;
...
MYBOOL MPS_readBAS(lprec *lp, int typeMPS, char *filename, char *info)
{
  ...
  while(fgets(line, BUFSIZ - 1, input)) {
    ...
    if(line[0] != ' ') {
      ...
      in = MPS_getnameidx(lp, field2, _FALSE);
#ifdef OldNameMatch
      if(in < 0)
      ...
      if(field1[0] == 'X') {
        ...
        ib = in;
        in = MPS_getnameidx(lp, field3, _FALSE);
#ifdef OldNameMatch
        if(in < 0)
