...
MYBOOL __WINAPI read_basis(lprec *lp, char *filename, char *info)
{
  ...
  if(typeMPS) {
    ...
    lp->var_basic[0] = _FALSE; /* Set to signal that this is a non-default basis */
  }
...
#define PricerDefaultOpt 1
  ...
  lp->verbose           = NORMAL;
  lp->print_sol         = _FALSE;       /* Can be _FALSE, TRUE, AUTOMATIC (only non-zeros printed) */
  lp->spx_trace         = _FALSE;
  lp->lag_trace         = _FALSE;
  lp->bb_trace          = _FALSE;
}
...
MYBOOL __WINAPI set_var_branch(lprec *lp, int colnr, int branch_mode)
{
  ...
  if(lp->bb_varbranch == NULL) {
    ...
    if(branch_mode == BRANCH_DEFAULT)
      return( TRUE );
    allocMYBOOL(lp, &lp->bb_varbranch, lp->columns_alloc, _FALSE);
...
{
  set_infiniteex(lp, infinite, _FALSE);
}
...
MYBOOL __WINAPI set_mat(lprec *lp, int rownr, int colnr, REAL value)
{
  ...
  if(rownr == 0) {
    ...
    return( mat_setvalue(lp->matA, rownr, colnr, value, _FALSE) );
}
...
# endif
  ...
  lp->source_is_file    = _FALSE;
  lp->model_is_pure     = TRUE;
  lp->model_is_valid    = _FALSE;
  lp->spx_status        = NOTRUN;
  lp->lag_status        = NOTRUN;

  lp->workarrays = mempool_create(lp);
  lp->wasPreprocessed   = _FALSE;
  lp->wasPresolved      = _FALSE;
  ...
  lp->basis_valid       = _FALSE;
  lp->simplex_mode      = SIMPLEX_DYNAMIC;
  lp->scaling_used      = _FALSE;
  lp->columns_scaled    = _FALSE;
  lp->P1extraDim        = 0;
  lp->P1extraVal        = 0.0;
...
lprec* __WINAPI copy_lp(lprec *lp)
{
  ...
  if(!allocINT(lp, &idx, lp->rows+1, _FALSE) ||
     !allocREAL(lp, &val, lp->rows+1, _FALSE))
    goto Finish;
  ...
  set_use_names(newlp, _FALSE, is_use_names(lp, _FALSE));
  ...
  set_mip_gap(newlp, _FALSE, get_mip_gap(lp, _FALSE));
  ...
  if(lp->solvecount > 0) {
    ...
    if(lp->duals != NULL) {
      allocREAL(newlp, &newlp->duals, newlp->sum_alloc+1, _FALSE);
...
STATIC MYBOOL memopt_lp(lprec *lp, int rowextra, int colextra, int nzextra)
{
  ...
  if(status) {
    ...
    status = inc_lag_space(lp, rowalloc, _FALSE) &&
...
*/
  ...
  shift_basis(lp, lp->rows+base, delta, usedmap, _FALSE);
  if(SOS_count(lp) > 0)
    SOS_shift_col(lp->SOS, 0, base, delta, usedmap, _FALSE);
  shift_rowcoldata(lp, lp->rows+base, delta, usedmap, _FALSE);
...
STATIC MYBOOL inc_col_space(lprec *lp, int deltacols)
{
  ...
  if(lp->columns+deltacols >= lp->columns_alloc) {
    ...
    inc_rowcol_space(lp, deltacols, _FALSE);
...
MYBOOL __WINAPI str_set_obj_fn(lprec *lp, char *row_string)
{
  ...
  allocREAL(lp, &arow, lp->columns + 1, _FALSE);
  p = row_string;
  for(i = 1; i <= lp->columns; i++) {
...
MYBOOL __WINAPI str_add_lag_con(lprec *lp, char *row_string, int con_type, REAL rhs)
{
  ...
  allocREAL(lp, &a_row, lp->columns + 1, _FALSE);
  p = row_string;
...
MYBOOL __WINAPI str_add_column(lprec *lp, char *col_string)
{
  ...
  allocREAL(lp, &aCol, lp->rows + 1, _FALSE);
  p = col_string;
...
STATIC MYBOOL del_varnameex(lprec *lp, hashelem **namelist, _hashtable *ht, int varnr, LLrec *varmap)
{
  int i, n;
...
MYBOOL __WINAPI set_var_weights(lprec *lp, REAL *weights)
{
  ...
  if(weights != NULL) {
    int n;
    allocINT(lp, &lp->var_priority, lp->columns_alloc, _FALSE);
    for(n = 0; n < lp->columns; n++) {
      lp->var_priority[n] = n+1;
    }
    n = sortByREAL(lp->var_priority, weights, lp->columns, 0, _FALSE);
  }
  return(TRUE);
}
...
/* Experimental automatic variable ordering/priority setting */
{
  ...
  if(is_bb_mode(lp, NODE_AUTOORDER) &&
    ...
    allocINT(lp, &colorder, lp->columns+1, _FALSE);
    ...
    i = getMDO(lp, NULL, colorder, NULL, _FALSE);

    /* Map to variable weight */
    allocREAL(lp, &rcost, lp->columns+1, _FALSE);
...
{
  maximize = (MYBOOL) (maximize != _FALSE);
...
MYBOOL __WINAPI set_constr_type(lprec *lp, int rownr, int con_type)
{
  ...
  if(oldchsign != is_chsign(lp, rownr)) {
    ...
    if(mat->is_roworder)
      mat_multcol(mat, rownr, -1, _FALSE);
    else
...
int __WINAPI get_rowex(lprec *lp, int rownr, REAL *row, int *colno)
{
  ...
  if((rownr == 0) || !mat_validate(lp->matA)) {
    ...
    for(; i < ie; i++) {
      j = ROW_MAT_COLNR(i);
      a = get_mat_byindex(lp, i, TRUE, _FALSE);
...
/*  allocREAL(lp, &this_rhs, lp->rows + 1, TRUE); */
  ...
  mempool_releaseVector(lp->workarrays, (char *) this_rhs, _FALSE);
/*  FREE(this_rhs); */
  return(TRUE);
}
...
MYBOOL rename_var(lprec *lp, int varindex, char *new_name, hashelem **list, _hashtable **ht)
{
  hashelem *hp;
  MYBOOL   newitem;
...
{
  if(isrow)
    return( find_row(lp, varname, _FALSE) );
  else
    return( find_var(lp, varname, _FALSE) );
}
...
char * __WINAPI get_origrow_name(lprec *lp, int rownr)
{
  ...
  if(lp->names_used && lp->use_row_names && (lp->row_name[rownr] != NULL) &&
    ...
    if(lp->rowcol_name == NULL)
      if (!allocCHAR(lp, &lp->rowcol_name, 20, _FALSE))
        return(NULL);
    ptr = lp->rowcol_name;
    if(newrow)
...
/* Note that this function is similar to the compute_reducedcosts function in lp_price.c */
{
  ...
  if(!get_colIndexA(lp, target, coltarget, _FALSE)) {
    mempool_releaseVector(lp->workarrays, (char *) coltarget, _FALSE);
    return(_FALSE);
  }
    ...
    for(; jb < je; jb++) {
      ...
      if(jj == pivcolnr) {
        if(rownr == 0)
          ...
          *pivcolval = get_mat_byindex(lp, jb, TRUE, _FALSE);
        continue;
      }
      if(!is_int(lp, jj))
        continue;
      ...
      if(rownr == 0)
        ...
        rowval = get_mat_byindex(lp, jb, TRUE, _FALSE);
      if(rowval > 0)
        (*plucount)++;
...
/*    if(((*rhsptr) < lp->lowbo[*idxptr]-tol) || ((*rhsptr) > lp->upbo[*idxptr]+tol)) */
  ...
  if(feasibilitygap != NULL) {
    if(feasible)
      ...
      *feasibilitygap = feasibilityOffset(lp, _FALSE);
  }
...
STATIC MYBOOL isDualFeasible(lprec *lp, REAL tol, int *boundflipcount, int infeasibles[], REAL *feasibilitygap)
{
  ...
  if((infeasibles != NULL) || (boundflipcount != NULL)) {
    ...
    f = compute_dualslacks(lp, target, &dcol, &nzdcol, _FALSE);
    if(nzdcol != NULL)
    ...
    f = compute_dualslacks(lp, target, NULL, NULL, _FALSE);
...
{
  lp->basis_valid = _FALSE;   /* Causes reinversion at next opportunity */
}
...
/* Explicitly set working bounds to given vectors without pushing or popping */
{
  ...
  if(ok) {
    ...
    if(lp->bb_bounds != NULL)
      lp->bb_bounds->UBzerobased = _FALSE;
    set_action(&lp->spx_action, ACTION_REBASE);
  }
  set_action(&lp->spx_action, ACTION_RECOMPUTE);
  return( ok );
...
/* Save the ingoing basis and push it onto the stack */
{
  ...
  if((newbasis != NULL) &&
    ...
    allocMYBOOL(lp, &newbasis->is_lower,  sum,  _FALSE) &&
#else
    allocMYBOOL(lp, &newbasis->is_lower,  (sum + 8) / 8,  TRUE) &&
#endif
#if BasisStorageModel == 0
    allocMYBOOL(lp, &newbasis->is_basic,  sum,  _FALSE) &&
#endif
    allocINT(lp,    &newbasis->var_basic, lp->rows + 1, _FALSE)) {
...
#endif
    j = lp->rows + i;
    if(!SOS_is_marked(lp->SOS, 0, i) && !SOS_is_full(lp->SOS, 0, i, _FALSE)) {
...
STATIC int find_int_bbvar(lprec *lp, int *count, BBrec *BB, MYBOOL *isfeasible)
{
  ...
  rcostmode      = (MYBOOL) /* _FALSE */ (BB->lp->solutioncount > 0) && is_bb_mode(lp, NODE_RCOSTFIXING) ; /* 5/2/08 peno disabled NODE_RCOSTFIXING because it results in non-optimal solutions with some models */ /* 15/2/8 peno enabled NODE_RCOSTFIXING again because a fix is found. See lp_simplex.c NODE__RCOSTFIXING fix */
  pseudocostmode = is_bb_mode(lp, NODE_PSEUDOCOSTMODE);
  pseudocostsel  = is_bb_rule(lp, NODE_PSEUDOCOSTSELECT) ||
                   is_bb_rule(lp, NODE_PSEUDONONINTSELECT) ||
                   is_bb_rule(lp, NODE_PSEUDORATIOSELECT);
  ...
  allocINT(lp, &nonint, lp->columns + 1, _FALSE);
  n = 0;
  depthmax = -1;
  if(isfeasible != NULL)
    *isfeasible = TRUE;
  ...
  for(k = 1; (k <= lp->columns); k++) {
    ...
    if(!isINT) {
      ...
      valINT = solution_is_int(lp, i, _FALSE);
  ...
  if(rcostmode) {
    for(i = 1; (i <= lp->rows) && (BB->lastrcf == 0); i++) {
      ...
      if(lowbo[i] < upbo[i]) {
        bestvar = rcfbound_BB(BB, i, _FALSE, NULL, isfeasible);
        if(bestvar != FR)
          BB->lastrcf++;
      }
    }
                               ...
                               REAL *prow, int *nzprow, REAL *pcol, int *nzpcol, int *boundswaps)
{
  ...
  MYBOOL minitNow = _FALSE, minitStatus = ITERATE_MAJORMAJOR;
...
#if 0
  if(verify_solution(lp, _FALSE, my_if(minitNow, "MINOR", "MAJOR")) >= 0) {
    if(minitNow)
...
#ifdef UseMaxValueInCheck
  allocREAL(lp, &maxvalue, lp->rows + 1, _FALSE);
  for(i = 0; i <= lp->rows; i++)
...
STATIC MYBOOL construct_duals(lprec *lp)
{
  ...
  /* Initialize */
  coltarget = (int *) mempool_obtainVector(lp->workarrays, lp->columns+1, sizeof(*coltarget));
  if(!get_colIndexA(lp, SCAN_USERVARS+USE_NONBASICVARS, coltarget, _FALSE)) {
    mempool_releaseVector(lp->workarrays, (char *) coltarget, _FALSE);
    return(_FALSE);
  }
  ...
  if(((lp->do_presolve & PRESOLVE_LASTMASKMODE) != PRESOLVE_NONE) &&
    ...
    presolve_rebuildUndo(lp, _FALSE);
  }
  ...
  if(!allocREAL(lp, &pcol, lp->rows + 1, TRUE) ||
    ...
    for(varnr=1; varnr<=lp->sum; varnr++) {
      ...
      if (!lp->is_basic[varnr]) {
        if (!fsolve(lp, varnr, pcol, workINT, epsvalue, 1.0, _FALSE)) {  /* construct one column of the tableau */
          ok = _FALSE;
          break;
        }
...
STATIC MYBOOL construct_sensitivity_obj(lprec *lp)
{
  ...
  if(!allocREAL(lp, &drow, lp->sum + 1, TRUE) ||
     !allocREAL(lp, &OrigObj, lp->columns + 1, _FALSE) ||
...
Abandon:
    ...
    for(i = 1; i <= lp->columns; i++) {
      ...
      /* only the coeff of the objective function of column i changes. */
        ...
        else if((lp->is_lower[varnr] != 0) == (is_maxim(lp) == _FALSE))
      ...
      /* all the coeff of the objective function change. Search the minimal change needed for further iterations */
        ...
        if(row_nr<=lp->rows) {       /* safety test; should always be found ... */
          ...
          if ((lp->is_lower[varnr] == 0) == (is_maxim(lp) == _FALSE)) {
            a = min1;
            min1 = min2;
            min2 = a;
          }
...
STATIC int verify_solution(lprec *lp, MYBOOL reinvert, char *info)
{
  ...
  allocINT(lp, &oldmap, lp->rows+1, _FALSE);
  allocINT(lp, &newmap, lp->rows+1, _FALSE);
  allocREAL(lp, &oldrhs, lp->rows+1, _FALSE);

  /* Get sorted mapping of the old basis */
  for(i = 0; i <= lp->rows; i++)
    oldmap[i] = i;
  if(reinvert) {
    allocINT(lp, &refmap, lp->rows+1, _FALSE);
  ...
  if(reinvert)
    invert(lp, INITSOL_USEZERO, _FALSE);
  else
...
STATIC int identify_GUB(lprec *lp, MYBOOL mark)
{
  ...
  for(i = 1; i <= lp->rows; i++) {
    ...
    for(jb = mat->row_end[i-1]; jb < je; jb++) {
      ...
      mv = get_mat_byindex(lp, jb, TRUE, _FALSE);
...
#define fullybounded _FALSE
   ...
   /* Then deal with -+, full-range/FREE variables by creating a helper variable */
      ...
      if(lp->var_is_free[j] <= 0) { /* If this variable wasn't split yet ... */
        ...
        if(new_column == NULL) {
          if(!allocREAL(lp, &new_column, lp->rows + 1, _FALSE) ||
             !allocINT(lp, &new_index, lp->rows + 1, _FALSE)) {
            ok = _FALSE;
            break;
          }
        }
