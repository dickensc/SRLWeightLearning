...
MYBOOL __WINAPI read_params(lprec *lp, char *filename, char *options)
{
  ...
  FILE *fp;
  _hashtable *hashfunctions, *hashparameters;
  hashelem *hp;
  ...
  if((fp = ini_open(filename)) == NULL)
    ...
    /* create _hashtable of all callable commands to find them quickly */
    ...
    /* create _hashtable of all arguments to find them quickly */
    ...
    while((ret) && (looping)) {
      line++;
      switch(ini_readdata(fp, buf, sizeof(buf), _FALSE)) {
        case 0: /* End of file */
          looping = _FALSE;
          break;
        case 1: /* header */
