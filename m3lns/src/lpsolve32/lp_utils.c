...
REAL *cloneREAL(lprec *lp, REAL *origlist, int size)
{
  ...
  size += 1;
  if(allocREAL(lp, &newlist, size, _FALSE))
...
MYBOOL *cloneMYBOOL(lprec *lp, MYBOOL *origlist, int size)
{
  ...
  size += 1;
  if(allocMYBOOL(lp, &newlist, size, _FALSE))
    MEMCOPY(newlist, origlist, size);
  return(newlist);
}
int *cloneINT(lprec *lp, int *origlist, int size)
{
  ...
  size += 1;
  if(allocINT(lp, &newlist, size, _FALSE))
    MEMCOPY(newlist, origlist, size);
  return(newlist);
}
...
STATIC MYBOOL verifyLink(LLrec *linkmap, int itemnr, MYBOOL doappend)
{
  ...
  testmap = cloneLink(linkmap, -1, _FALSE);
  if(doappend) {
...
STATIC MYBOOL unpackPackedVector(PVrec *PV, REAL **target)
{
  ...
  if(target == NULL)
    return( _FALSE );
  if(*target == NULL)
    allocREAL(NULL, target, PV->startpos[PV->count], _FALSE);
...
{
  index = searchFor(index, PV->startpos, PV->count, 0, _FALSE);
  index = abs(index)-1;
  if(index >= 0)
