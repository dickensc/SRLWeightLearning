...
STATIC SOSrec *create_SOSrec(SOSgroup *group, char *name, int type, int priority, int size, int *variables, REAL *weights)
{
  ...
  {
    allocCHAR(group->lp, &SOS->name, (int) (strlen(name)+1), _FALSE);
...
STATIC int make_SOSchain(lprec *lp, MYBOOL forceresort)
{
  ...
  allocINT(lp, &lp->sos_priority, n, _FALSE);
  allocREAL(lp, &order, n, _FALSE);
  ...
  hpsortex(order, n, 0, sizeof(*order), _FALSE, compareREAL, lp->sos_priority);
  FREE(order);
...
int SOS_member_index(SOSgroup *group, int sosindex, int member)
{
  ...
  n = searchFor(member, SOS->membersSorted, n, 0, _FALSE);
  if(n >= 0)
...
int SOS_is_member(SOSgroup *group, int sosindex, int column)
{
  ...
  if(sosindex == 0) {
   ...
   /* Signal active status if found, otherwise return _FALSE */
    if(i > 0) {
      list = group->sos_list[sosindex-1]->members;
      if(list[i] < 0)
        n = -TRUE;
