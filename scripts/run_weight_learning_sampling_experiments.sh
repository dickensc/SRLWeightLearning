...
function run_example() {
    ...
    if [[ -e "${out_path}" ]]; then
        ...
        pushd . > /dev/null
            ...
            /usr/bin/time -v --output="${time_path}" ./run_wl.sh "${example_name}" "${fold}" "${SEED}" "${alpha}" "sampling_study" "${wl_method}" "${evaluator}" "${out_directory}" "${TRACE_LEVEL}" > "$out_path" 2> "$err_path"
#              ./run_wl.sh "${example_name}" "${fold}" "${SEED}" "${alpha}" "sampling_study" "${wl_method}" "${evaluator}" "${out_directory}" "${TRACE_LEVEL}"> "$out_path" 2> "$err_path"
        popd > /dev/null
    fi
